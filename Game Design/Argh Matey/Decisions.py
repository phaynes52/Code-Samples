...
def danceCircle() :
    ...
    if dance == "bust a move":
        ...
        raw_input("You freeze up and run out of the circle. The crowd is angry. You got their hopes up only to crush them. Their anger manifests and people start throwing things at you. It starts with ice, then bottles, then chairs. It hurts. You should have just danced. \m-10 health\n")
        health = health - 10
        if health <= 0:
            print("Your health is critically low and you can no longer continue. You pass out. Thank you for playing. Goodbye.")
            quit()
...
def landho():
    ...
    choice = raw_input("Captain Blackbreath: 'Our crew is headed toward Cifuentes on the other side of that island. On this side of the island is a small group of British soldiers. You can come with us and we'll do our best to help you out, or we can send you with a dingy to the island and you can see if the British can help you. Up to you.' \n (Go to the island / Stay with the pirates) ").lower()  
    while choice not in ["go to the island", "stay with the pirates"] :
        choice = raw_input("Shiver me timbers, who would try to do that??. Please enter a choice from the options provided. \n(Go to the island / stay with the pirates) ").lower()
    if choice == "go to the island":
        theBritishAreComing()
    if choice == "stay with the pirates":
        thePiratesLifeForMe()
    return

def theBritishAreComing():
    raw_input("You tell the captain that you want to take you chances on the island and he has the crew prepare a boat for your departure. \nCaptain Blackbreath: 'Good luck out there. The British have never been kind to us but I've got a good feeling about you.'")
    ...
    if duel == "go with the soldier" :
        ...
        fight(70, 6, 4)
        #drops some shit if you win - restote some health and level up strength.
    return 


def thePiratesLifeForMe():
    return
